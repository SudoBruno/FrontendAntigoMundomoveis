...
export default function DefectLevel() {
  class SearchTable extends React.Component {
    ...
    getColumnSearchProps = (dataIndex) => ({
      ...
      filterIcon: (filtered) => (
        <SearchOutlined style={{ level: filtered ? '#1890ff' : undefined }} />
      ),
    ...
    render() {
      ...
      return <Table columns={columns} dataSource={level} />;
    }
  }
  ...
  async function handleRegister(e) {
    ...
    try {
      if (id === 0) {
        try {
          const response = await api.post('defectLevel', data);
        ...
        try {
          const response = await api.put('defectLevel', data);
  ...
  async function handleDeleteFunction(id) {
    try {
      const response = await api.delete(`defectLevel/${id}`);
      setLevel(level.filter((level) => level.id !== id));
      openNotificationWithIcon(
        'success',
