...
export default function ExpeditionWarehouse() {
  ...
  async function handleRegister(e) {
    ...
    const data = {
      ...
      name,
      locked,
      expedition_warehouse_id: expeditionWarehouseId,
    };
    if (name == '') {
    ...
    try {
      if (id === 0) {
        try {
          const response = await api.post('expedition/street', data);
        ...
        try {
          const response = await api.put('expedition/street', data);
  ...
  async function handleDeleteFunction(id) {
    try {
      const response = await api.delete(`expedition/street/${id}`);
